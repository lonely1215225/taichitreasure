...
public class CipherUtils {
    ...
    public static String encrypt(InputStream is, OutputStream out, String publicKey) throws Exception {
        ...
        PublicKey aPublic = rsa.generatePublic(keySpec);
    ...
    // 关闭IO资源
    private static void closeIO(InputStream is, OutputStream out, CipherInputStream cis) {
        try {
            if (cis != null)
                cis.close();
            if (is != null)
    ...
    public static boolean verify(String SID, String signature, String publicKey) {
        ...
        } catch (NoSuchPaddingException | NoSuchAlgorithmException | InvalidKeyException | IllegalBlockSizeException | InvalidKeySpecException | BadPaddingException e) {
            e.printStackTrace();
        }
        assert srcSID != null;
        return srcSID.equals(SIDBASE64);
